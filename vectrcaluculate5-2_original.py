...
def vectr(d, a ,r, R,n1,n2 ) : # 関数を定義
 #d(xd yd zd) :方向ベクトル，a(xa ya za): 位置ベクトル，r(xr yr zr) :球の中心 球の半径:R,屈折率:n1,n2
 #At^2+2Bt+C=0より
 A =d[0]**2+d[1]**2+d[2]**2
 ...
 if B**2-A*C<0:
     ...
     t=np.array(((-B+(B**2-A*C)**0.5)/A, (-B-(B**2-A*C)**0.5)/A),float)
...
for x in range (0,99,1):
 D0[x]=np.array((0,np.sin(deg[x]*np.pi/180),np.cos(deg[x]*np.pi/180)))#画角
 a0=np.array((0,0,0))
 d0=100
 n0=1
 ...
 data=np.array([(11.050,1.74,5.50),
              ...
              (27.530,1.74,1.600),
              (-18.100,1,46.700),
              (1000000,1,10000)],float)#ここにパラメータを入力（axis=0方向:第i面　axis=1方向:曲率，屈折率，厚み）

 number=np.shape(data)
 number=number[0]#面の数
 
 #数値置き場
 list1=np.zeros((number,3))#球の中心
 list2=np.zeros((number,3))#交点
 list3=np.zeros((number,3))#屈折ベクトル
 ...
 for i in range (0,number-1,1):
     rx=np.array((list1[i,0],list1[i,1],d0+np.sum(data[:i+1,2])+data[i+1,0]))#i面の球の中心
     px=vectr(list3[i,:],list2[i,:],rx,data[i+1,0],data[i,1],data[i+1,1])#i-1_i面の光線追跡
 ...
 zz[x]=point0[x,2]-point0[0,2]
 print(zz)
 
 z_imagey[x]=np.dot(point[x,2],point[x,1])/ref_vector[x,2]
 #print(z_imagey)
 SCA[x]=point0[0,2]-(z_imagey[x]-zz[x])
